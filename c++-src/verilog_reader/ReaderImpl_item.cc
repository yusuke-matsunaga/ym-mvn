...
)
{
  ...
  if ( has_edge_event ) {
	...
	if ( node == node1 ) {
	  ...
	  found = true;
	  ProcEnv proc_env{mGlobalEnv};
	  auto ctrl{new AsyncControl{proc_env}};
	  ctrl->mNode = node;
	  ctrl->mPol = pol;
